...
'callback'    查询结果
*/
    findDocuments: function (option, callback) {
        var sorter = option.sorter || {};
        var skipNum = option.skipNum || 0;
        var limitNum = option.limitNum || 0;
        var collection = option.db.collection(option.collection);
        var fields = option.fields?option.fields:{};
        var num
        // console.log('skipNum: '+skipNum+" limitNum: "+limitNum)
        collection.find().count({}, function (err, count) {
            if (err) { console.log(err) }
            else{num = count}
        });
        collection.find(option.filter,fields).sort(sorter).skip(skipNum).limit(limitNum).toArray(function (err, docs) {
			if(err){console.log(err)}
            else {
				callback(docs,num);
			}
...
'callback'    删除结果
*/
	 removeOneDocument : function(db,collection,filter){
		var collection = db.collection(collection);
...
*/
 	 updateOneDocument : function(db,collection,data){
 		var collection = db.collection(collection);
 ...
 //________________________________________________________________________________________________________________
	...
	*/
	...
	//filter: criteria of finding operation  
	//filter: 查询条件
    forage: function (option, callback) {
        //url, collection, filter
        MongoClient.connect(option.url, function (err, db) {
            option.db = db;
			if(err){console.log(err)}
			else{
                databaseManipulate.findDocuments(option, function (doc,num) {
                    callback(doc, num);
					db.close();
				})
			}
	...
	//修改数据
	exchange : function (url,collection,data){
		MongoClient.connect(url, function(err,db){
			if(err){console.log(err)}
			else{
				databaseManipulate.updateOneDocument(db,collection,data,function(){
					db.close();
				})
			}
		});
